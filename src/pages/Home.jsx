  ...
  return (
    <>
      ...
      <div className=" cardContainer container row  justify-content-evenly align-items-center mt-5 mx-auto">
        {!newData && (<div className="d-flex  justify-content-center mt-2">
          <img  className="w-50 mw-75" src={img1} alt="" />

        </div>)}
        {newData?.length === 0 && (<h1 
        className="">The News can not be found</h1>) }

        {newData?.map((item) => {
          const {urlToImage,title,description,publishedAt,source:{name},url} =item
          return (
            <div className="card col-sm-12 col-md-6 col-lg-4 p-3   " key={url}>
              <img loading="lazy" src={urlToImage ? urlToImage : defaultImage }  className="card-img-top" alt="..." />
              <div className="card-body">
                <h5 className="card-title">{title} </h5>
                <p className="card-text">{readMore ? description : description.slice(0,100)+ "..."}
                <button className="border border-0 fw-bolder text-capitalize bg-white text-dark" onClick={()=>setReadMore(!readMore) } >{readMore ? "show less" : "readmore" } </button>
                </p>
              ...
              <div className="card-body">
                <button 
                 onClick={() => {
                  currentUser ?   navigate("detail",{state:item,replace:false}) :toastWarnNotify('Please log in to see detail')
